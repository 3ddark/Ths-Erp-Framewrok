unit Ths.Erp.Database.Table.AyarTeklifDurum;
...
constructor TAyarTeklifDurum.Create(OwnerDatabase:TDatabase);
begin
  inherited Create(OwnerDatabase);
  TableName := 'ayar_teklif_durum';
  SourceCode := '1000';
...
procedure TAyarTeklifDurum.SelectToDatasource(pFilter: string; pPermissionControl: Boolean=True);
begin
...
procedure TAyarTeklifDurum.SelectToList(pFilter: string; pLock: Boolean; pPermissionControl: Boolean=True);
begin
  if IsAuthorized(ptRead, pPermissionControl) then
  begin
    if (pLock) then
...
procedure TAyarTeklifDurum.Insert(out pID: Integer; pPermissionControl: Boolean=True);
begin
...
procedure TAyarTeklifDurum.Update(pPermissionControl: Boolean=True);
begin
...
procedure TAyarTeklifDurum.Clear();
begin
  inherited;
...
function TAyarTeklifDurum.Clone():TTable;
begin
  Result := TAyarTeklifDurum.Create(Database);

  Self.Id.Clone(TAyarTeklifDurum(Result).Id);

  FDeger.Clone(TAyarTeklifDurum(Result).FDeger);
  FAciklama.Clone(TAyarTeklifDurum(Result).FAciklama);
  FIsActive.Clone(TAyarTeklifDurum(Result).FIsActive);
end;
