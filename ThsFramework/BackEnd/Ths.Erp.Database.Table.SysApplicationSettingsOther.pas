...
begin
  ...
  FIsEdefterAktif := TFieldDB.Create('is_edefter_aktif', ftBoolean, False);
  ...
  FIsBolumAmbardaUretimYap := TFieldDB.Create('is_bolum_ambarda_uretim_yap', ftBoolean, False);
  FIsUretimMuhasebeKaydiOlustursun := TFieldDB.Create('is_uretim_muhasebe_kaydi_olustursun', ftBoolean, False);
  FIsStokSatimdaNegatifeDusebilir := TFieldDB.Create('is_stok_satimda_negatife_dusebilir', ftBoolean, False);
  FIsMalSatisSayilariniGoster := TFieldDB.Create('is_mal_satis_sayilarini_goster', ftBoolean, False);
  FIsPcbUretim := TFieldDB.Create('is_pcb_uretim', ftBoolean, False);
  FIsProformaNoGoster := TFieldDB.Create('is_proforma_no_goster', ftBoolean, False);
  FIsSatisTakip := TFieldDB.Create('is_satis_takip', ftBoolean, False);
  FIsHammaddeGiriseGoreSirala := TFieldDB.Create('is_hammadde_girise_gore_sirala', ftString, '');
  FIsUretimEntegrasyonHammaddeKullanimHesabiIscilikle := TFieldDB.Create('is_uretim_entegrasyon_hammadde_kullanim_hesabi_iscilikle', ftBoolean, False);
  FIsTahsilatListesiVirmanli := TFieldDB.Create('is_tahsilat_listesi_virmanli', ftBoolean, False);
  FIsOrtalamaVadeSifirsaSevkiyataIzinVerme := TFieldDB.Create('is_ortalama_vade_0_ise_sevkiyata_izin_verme', ftBoolean, False);
  FIsSiparisteTeslimTarihiYazdir := TFieldDB.Create('is_sipariste_teslim_tarihi_yazdir', ftBoolean, False);
  FIsTeklifAyrintilariniGoster := TFieldDB.Create('is_teklif_ayrintilarini_goster', ftBoolean, False);
  FIsFaturaIrsaliyeNoSifirlaBaslasin := TFieldDB.Create('is_fatura_irsaliye_no_0_ile_baslasin', ftBoolean, False);
  FIsExcelEkliIrsaliyeYazdirma := TFieldDB.Create('is_excel_ekli_irsaliye_yazdirma', ftBoolean, False);
  FIsAmbarTransferNumarasiOtomatikGelsin := TFieldDB.Create('is_ambar_transfer_numara_otomatik_gelsin', ftBoolean, False);
  FIsAmbarTransferOnayliCalissin := TFieldDB.Create('is_ambar_transfer_onayli_calissin', ftBoolean, False);
end;
