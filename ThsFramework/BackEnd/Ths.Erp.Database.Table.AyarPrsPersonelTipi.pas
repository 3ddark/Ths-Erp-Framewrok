unit Ths.Erp.Database.Table.AyarPrsPersonelTipi;
...
type
  TAyarPrsPersonelTipi = class(TTable)
  private
  ...
  begin
    ...
    begin
      ...
      SQL.Text := Database.GetSQLSelectCmd(TableName, [
        ...
        getRawDataByLang(TableName, FPersonelTipi.FieldName),
...
procedure TAyarPrsPersonelTipi.SelectToList(pFilter: string; pLock: Boolean; pPermissionControl: Boolean=True);
begin
  if IsAuthorized(ptRead, pPermissionControl) then
  begin
    if (pLock) then
...
procedure TAyarPrsPersonelTipi.Insert(out pID: Integer; pPermissionControl: Boolean=True);
begin
...
procedure TAyarPrsPersonelTipi.Update(pPermissionControl: Boolean=True);
begin
...
function TAyarPrsPersonelTipi.Clone():TTable;
begin
  Result := TAyarPrsPersonelTipi.Create(Database);

  Self.Id.Clone(TAyarPrsPersonelTipi(Result).Id);

  FPersonelTipi.Clone(TAyarPrsPersonelTipi(Result).FPersonelTipi);
  FIsActive.Clone(TAyarPrsPersonelTipi(Result).FIsActive);
end;
