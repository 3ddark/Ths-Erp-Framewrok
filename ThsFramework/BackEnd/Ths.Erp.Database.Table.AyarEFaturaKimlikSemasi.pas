unit Ths.Erp.Database.Table.AyarEFaturaKimlikSemasi;
...
type
  TAyarEFaturaKimlikSemasi = class(TTable)
  private
    FDeger: TFieldDB;
    FAciklama: TFieldDB;
    FIsActive: TFieldDB;
...
constructor TAyarEFaturaKimlikSemasi.Create(OwnerDatabase:TDatabase);
begin
  inherited Create(OwnerDatabase);
  TableName := 'ayar_efatura_kimlik_semasi';
  SourceCode := '1000';
...
procedure TAyarEFaturaKimlikSemasi.SelectToDatasource(pFilter: string; pPermissionControl: Boolean=True);
begin
...
procedure TAyarEFaturaKimlikSemasi.SelectToList(pFilter: string; pLock: Boolean; pPermissionControl: Boolean=True);
begin
  if IsAuthorized(ptRead, pPermissionControl) then
  begin
    if (pLock) then
...
procedure TAyarEFaturaKimlikSemasi.Insert(out pID: Integer; pPermissionControl: Boolean=True);
begin
...
procedure TAyarEFaturaKimlikSemasi.Update(pPermissionControl: Boolean=True);
begin
...
function TAyarEFaturaKimlikSemasi.Clone():TTable;
begin
  Result := TAyarEFaturaKimlikSemasi.Create(Database);

  Self.Id.Clone(TAyarEFaturaKimlikSemasi(Result).Id);

  FDeger.Clone(TAyarEFaturaKimlikSemasi(Result).FDeger);
  FAciklama.Clone(TAyarEFaturaKimlikSemasi(Result).FAciklama);
  FIsActive.Clone(TAyarEFaturaKimlikSemasi(Result).FIsActive);
end;
