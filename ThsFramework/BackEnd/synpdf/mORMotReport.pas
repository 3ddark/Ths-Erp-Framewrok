...
(*
  ...
  *** BEGIN LICENSE BLOCK *****
  ...
  Portions created by Arnaud Bouchez for Synopse are Copyright (C) 2013
 ...
 **********************************************
  ...
  Modifications © 2009-2013 Arnaud Bouchez
...
{$endif}
    ...
    procedure BeginDoc(pFarkliYaziciKullan: Boolean = True);
    ...
    procedure DrawLineGrey(doubleline: boolean=false);
    /// draw a Dashed Line between the left & right margins
    procedure DrawLineCustom(left_pos:integer; length:integer; doubleline: boolean=false);
    procedure DrawLineRightMargin(length:integer; doubleline: boolean=false);
    procedure DrawLineLeftMargin(length:integer; doubleline: boolean=false);
...
begin
  ...
  with fPreviewSurface do begin
    ...
    if (fPages<>nil) and (cardinal(Page-1)<=cardinal(High(fPages))) then begin
...
procedure TGDIPages.BeginDoc(pFarkliYaziciKullan: Boolean = True);
var
  printer_select : TPrinterSetupDialog;
begin
  if pFarkliYaziciKullan then
  begin
    printer_select := TPrinterSetupDialog.Create(nil);
    try
      printer_select.Execute;
    finally
      printer_select.Free;
    end;
  end;
...
procedure TGDIPages.DrawLineGrey(doubleline: boolean);
var c  : Integer;
begin
  if Self=nil then exit; // avoid GPF
  CheckHeaderDone;
...
procedure TGDIPages.DrawLineCustom(left_pos:integer; length:integer; doubleline: boolean=false);
begin
  if Self=nil then exit; // avoid GPF
  CheckHeaderDone;
  LineInternal(fPageMarginsPx.left + left_pos, fPageMarginsPx.left + left_pos + length, doubleline);
  NewLine;
end;

procedure TGDIPages.DrawLineRightMargin(length:integer; doubleline: boolean=false);
begin
  if Self=nil then exit; // avoid GPF
  CheckHeaderDone;
  LineInternal(fPhysicalSizePx.x-fPageMarginsPx.right, fPhysicalSizePx.x-fPageMarginsPx.right - length, doubleline);
  NewLine;
end;

procedure TGDIPages.DrawLineLeftMargin(length:integer; doubleline: boolean=false);
begin
  if Self=nil then exit; // avoid GPF
  CheckHeaderDone;
  LineInternal(fPageMarginsPx.Left, fPageMarginsPx.Left + length, doubleline);
  NewLine;
end;
