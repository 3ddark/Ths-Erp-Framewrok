...
begin
  result := FHeight - FOffsetYDef - (FWorldOffsetY + ViewOffsetY(Y) * GetWorldFactorY) * FDevScale
end;
...
begin
  ...
  EMR_LINETO: begin
    E.NeedPen;
    if (not E.Canvas.FNewPath) and (Int64(Moved)<>MAX_INT64) then begin
      E.Canvas.MoveToI(Moved.X,Moved.Y);
      Int64(Moved) := MAX_INT64; // mark EMR_MOVETOEX processed
    end;
    E.Canvas.LineToI(PEMRLineTo(R)^.ptl.X,PEMRLineTo(R)^.ptl.Y);
    Moved := PEMRLineTo(R)^.ptl;
    E.fInLined := true;
  ...
  if (not brush.null) or (not pen.null) then begin
    ...
    if R^.iType in [EMR_POLYGON, EMR_POLYLINE] then begin
      ...
      Moved := PEMRPolyLine(R)^.aptl[PEMRPolyLine(R)^.cptl-1];
    end else begin
  ...
  EMR_POLYBEZIER: begin
    ...
    Moved := PEMRPolyBezier(R)^.aptl[PEMRPolyBezier(R)^.cptl-1];
    if not E.Canvas.FNewPath then
      if not pen.null then
  ...
  EMR_POLYBEZIERTO: begin
    if not pen.null then
      E.NeedPen;
    if not E.Canvas.FNewPath then
      if Int64(Moved)<>MAX_INT64 then begin
        E.Canvas.MoveToI(Moved.X,Moved.Y);
        Int64(Moved) := MAX_INT64; // processed
      end;
    ...
    Moved := PEMRPolyBezierTo(R)^.aptl[PEMRPolyBezierTo(R)^.cptl-1];
    if not E.Canvas.FNewPath then
      if not pen.null then
        E.Canvas.Stroke else
        E.Canvas.NewPath;
  ...
  EMR_POLYLINETO, EMR_POLYLINETO16: begin
    ...
    if R^.iType=EMR_POLYLINETO then begin
      ...
      Moved := PEMRPolyLineTo(R)^.aptl[PEMRPolyLineTo(R)^.cptl-1]; 
  ...
  EMR_INTERSECTCLIPRECT:
    ClipRgn := E.IntersectClipRect(E.Canvas.BoxI(PEMRIntersectClipRect(R)^.rclClip), ClipRgn);
  EMR_SETMAPMODE:
  ...
  EMR_SETPIXELV: begin
    ...
    // draw point
    E.Canvas.PointI(PEMRSetPixelV(R)^.ptlPixel.X, PEMRSetPixelV(R)^.ptlPixel.Y);
    Moved := Point(PEMRSetPixelV(R)^.ptlPixel.X, PEMRSetPixelV(R)^.ptlPixel.Y);
    E.Canvas.Stroke;
...
begin
  ...
  try
    ...
    with Canvas do begin
      Box := BoxI(Rect(xd, yd, wd+xd, hd+yd));
      ClipRc := GetClipRect;
    ...
    EMR_POLYPOLYGON, EMR_POLYPOLYLINE: begin
      ...
      for i := 1 to data^.nPolys do begin
        ...
        for j := 2 to data^.aPolyCounts[i-1] do begin
          ...
          DC[nDC].Moved := Point(pPolyPointsArray[PolyOffs].X, pPolyPointsArray[PolyOffs].Y);
          Inc(PolyOffs);
        end;
        Canvas.LineToI(pPolyPointsArray[PolyFirst].X, pPolyPointsArray[PolyFirst].Y);
        //DC[nDC].Moved := Point(pPolyPointsArray[PolyFirst].X, pPolyPointsArray[PolyFirst].Y);
      end;
    end;
    EMR_POLYPOLYGON16, EMR_POLYPOLYLINE16: begin
      ...
      for i := 1 to data16^.nPolys do begin
        ...
        for j := 2 to data16^.aPolyCounts[i-1] do begin
          ...
          DC[nDC].Moved := Point(pPolyPointsArray16[PolyOffs].X, pPolyPointsArray16[PolyOffs].Y);
          Inc(PolyOffs);
        end;
        Canvas.LineToI(pPolyPointsArray16[PolyFirst].X, pPolyPointsArray16[PolyFirst].Y);
        //DC[nDC].Moved := Point(pPolyPointsArray16[PolyFirst].X, pPolyPointsArray16[PolyFirst].Y);
      end;
    end;
  end;
...
begin
  fInitMetaRgn := Canvas.BoxI(ClientRect);
...
{$endif}
    ...
    if font.spec.angle<>0 then begin
      ...
      Canvas.SetTextMatrix(acos, asin, -asin, acos,
        ...
        Canvas.I2Y(R.emrtext.ptlReference.Y-Round(H*acos-W*asin)));
    end else begin
      acos := 0;
      asin := 0;
      PosX := R.emrtext.ptlReference.X-W;
      PosY := R.emrtext.ptlReference.Y-H;
    ...
    if font.LogFont.lfUnderline<>0 then
      DrawLine(R.emrtext.ptlReference, aSize / 8 / Canvas.GetWorldFactorX / Canvas.FDevScale);
    if font.LogFont.lfStrikeOut<>0 then
      DrawLine(R.emrtext.ptlReference, - aSize / 3 / Canvas.GetWorldFactorX / Canvas.FDevScale);
